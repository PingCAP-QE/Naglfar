...
func (a MemoryAllocator) Alloc(ctx *AllocContext) (err error) {
	...
	for machine, bindings := range ctx.Machines {
		...
		reserve, err = machine.Spec.Reserve.Memory.ToSize()
		if err != nil {
			return
		}
		...
		for _, resourceRef := range ctx.Relationship.Status.MachineToResources[machineKey] {
			var memoryBinding int64
			memoryBinding, err = resourceRef.Binding.Memory.ToSize()
			if err != nil {
				return
			}
